...
wchar_t* GetGameDirectory()
{
	...
	wcscat_s( gamePath, L"\\steamapps\\common\\Counter-Strike Global Offensive Beta - Dedicated Server" );
	wprintf( L"Dedicated server's directory: %s\n", gamePath );
...
int StartLauncher( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
	...
	g_hDedicated = LoadLibraryExW( L"dedicated.dll", NULL, LOAD_WITH_ALTERED_SEARCH_PATH );

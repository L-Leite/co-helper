...
static std::vector< PLH::IHook* > s_PolyHooks;

void PLH::DestroyAll()
{
	for ( auto& hook : s_PolyHooks )
	{
		switch ( hook->GetType() )
		{
			case HookType::X86Detour:
			case HookType::X64Detour:
			{
				Detour* detour = static_cast<Detour*>(hook);
				if ( detour->Hooked() )
				{
					hook->UnHook();
					delete hook;
			...
			case HookType::VTableSwap:
			{
				VTableSwap* tableswap = static_cast<VTableSwap*>(hook);
				if ( tableswap->Hooked() )
				{
					hook->UnHook();
					delete hook;
			...
			default:
				printf( "PLH::DestroyAll: unknown type %i!\n", hook->GetType() );
		}
	}
...
PLH::AbstractDetour::AbstractDetour( bool bRegister /*= true*/ ) :IHook(), m_NeedFree(false), m_Hooked(false)
{
	if ( bRegister )
		s_PolyHooks.push_back( this );
...
/*----------------------------------------------*/
#ifndef _WIN64
PLH::X86Detour::X86Detour( bool bRegister /*= true*/ ) : AbstractDetour( bRegister )
{
